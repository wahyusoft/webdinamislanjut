...
--
-- Host: 127.0.0.1
-- Generation Time: 05 Okt 2016 pada 14.27
-- Versi Server: 5.6.21
-- PHP Version: 5.5.19
...
BEGIN
  SELECT "Assalamualaikum" AS salam;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `jumlahEmployees`(OUT hasil INT)
BEGIN   SELECT COUNT(*) INTO hasil FROM employees;    
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `jumlahItemBarang`(pelanggan VARCHAR(5))
BEGIN 
  SELECT SUM(od.Quantity) 
     FROM orders AS o JOIN order_details AS od
     ON o.OrderID=od.OrderID 
     WHERE o.CustomerID=pelanggan; 
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `simple_sqls`(  )
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `totalItemBarang`(CID varchar(5))
BEGIN
	SELECT SUM(od.Quantity) 
		FROM orders AS o 
...
-- Fungsi
--
CREATE DEFINER=`root`@`localhost` FUNCTION `getStockbySupplierID`(ID INT(5)) RETURNS int(11)
BEGIN
    DECLARE Qty INT;
    SELECT SUM(UnitsInStock) INTO Qty FROM products WHERE SupplierID=ID;
    RETURN Qty;
  END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `yourOld`(born DATE) RETURNS int(11)
BEGIN    
    DECLARE Current_year, BirthDate INT;	                                                                                                                                                                                                                                                                                                    
    SET Current_year = YEAR(now());                                                                                                                                                                                                                                                                           
    SET BirthDate = YEAR(born);                                                                                                                                                                                                                                                                               
...
--
-- Stand-in structure for view `terjual97`
--
CREATE TABLE IF NOT EXISTS `terjual97` (
`Bulan` varchar(9)
,`Terjual` decimal(27,0)
);
-- --------------------------------------------------------
...
--
-- Struktur untuk view `bestseller`
--
DROP TABLE IF EXISTS `bestseller`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `bestseller` AS (select `c`.`CategoryName` AS `CategoryName`,sum(`od`.`Quantity`) AS `qty` from (((`categories` `c` join `products` `p` on((`c`.`CategoryID` = `p`.`CategoryID`))) join `order_details` `od` on((`od`.`ProductID` = `p`.`ProductID`))) join `orders` `o` on((`o`.`OrderID` = `od`.`OrderID`))) where ((year(`o`.`OrderDate`) = 1996) and (month(`o`.`OrderDate`) between 7 and 12)) group by 1 order by 2 desc);

-- --------------------------------------------------------

--
-- Struktur untuk view `terjual97`
--
DROP TABLE IF EXISTS `terjual97`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `terjual97` AS (select monthname(`o`.`OrderDate`) AS `Bulan`,sum(`od`.`Quantity`) AS `Terjual` from (`order_details` `od` join `orders` `o` on((`o`.`OrderID` = `od`.`OrderID`))) where (year(`o`.`OrderDate`) = 1997) group by 1 order by month(`o`.`OrderDate`));
